...
class PromptFieldTracker:
    ...
    def track_positions(self, history_prompt, candidate_data, query_prompt):
        ...
        for i in range(len(all_chunk_ids)):
            if i == 0:
                self.input_ids += all_chunk_ids[i]
            else:
                self.input_ids += all_chunk_ids[i]
...
def load_user_data(user_id):
    ...
    Reads a single reviewer JSON file and splits the first 3 items as history
    and the rest as candidates. Supports both "user_XXXX" and "reviewer_XXXX"
    ID formats.
    """
