...
class PromptFieldTracker:
    ...
    def __init__(self, tokenizer, debug: bool = False):
        """
        Initialize the PromptFieldTracker
    ...
    def track_positions(self, prefix_prompt: str, track_data: List[Dict[str, Any]], query_prompt: str) -> Tuple[List[int], List[List[int]]]:
        """
        Track the position of each item JSON field in the complete prompt

        This method builds the complete prompt by concatenating the prefix prompt,
        all items to be tracked, and the query prompt. It then tracks the token
        positions of each field in each item.

        Args:
            prefix_prompt: System prompt or other prefix text
            track_data: List of item dictionaries to be tracked
            query_prompt: Query prompt to append after all items

        Returns:
            Tuple containing:
                - Complete input_ids for the entire prompt
                - all_chunk_ids for assembling KV cache (list of token ID chunks)
        """
    ...
    def verify_positions(self):
        ...
        Returns:
            bool: True if all positions are accurate, False otherwise
        """
    ...
    def visualize_positions(self):
        ...
        This method adds markers to the text to show where each field is located
        """
        if self.input_ids is None:
            if self.debug:
        ...
        # Use different markers to represent different fields
        markers = {
            'title': '↓T↓',
            'price': '↓P↓',
            'category': '↓C↓',
    ...
    def _is_similar(self, expected: Any, actual: str) -> bool:
        """
        Check if two values are similar, accounting for tokenizer differences

        This method compares an expected value with the actual decoded text,
        considering various formats and encodings that might differ due to
        tokenization or JSON serialization.

        Args:
            expected: The expected value (can be any type)
            actual: The actual decoded string from tokens

        Returns:
            bool: True if the values are considered similar, False otherwise
        """
    ...
    def get_field_positions(self) -> List[Dict[str, Any]]:
        """
        Get all field position information

        Returns:
            List of dictionaries containing position information for each item and its fields
        """
        return self.item_positions

def load_user_data(user_id: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Load user history and candidates from reviewer_data_processed_strict.

    Reads a single reviewer JSON file and returns the first 3 records as
    history and the rest as candidates. Accepts both "user_XXXX" and
    "reviewer_XXXX" identifiers.
    """
...
def generate_recommendation_with_cacheblend(user_id: str) -> Tuple[int, int]:
    """
    Generate recommendation prompt and track token positions for a user

    Args:
        user_id: The ID of the user to generate recommendations for

    Returns:
        Tuple containing (full prompt length, important tokens length)
    """
    # Load user data
...
{{\"rank\": \"[] > [] .. > []\"}} Do not say any word except this JSON format."""
    ...
    tracker = PromptFieldTracker(tokenizer, debug=False)

    # Log information about loaded data
    print(f"Number of loaded history items: {len(history_data)}")
    print(f"Number of loaded candidate items: {len(candidate_data)}")

    # Track positions of fields in the prompt
    input_ids, _ = tracker.track_positions(prefix_prompt, all_items, query_prompt)

    # Get query token IDs for position tracking
    query_ids = tokenizer.encode(query_prompt)[1:]

    # Collect all important token positions
    position = []

    # Add field positions
    for item_info in tracker.get_field_positions():
        for _, pos in item_info['fields'].items():
            start, end = pos['start'], pos['end']
            position.extend(list(range(start, end + 1)))

    # Add query prompt positions
    ...
    return len(input_prompt), len(position)

if __name__ == "__main__":
    # Get all user directories from the dataset folder
    dataset_dir = os.path.join(os.path.dirname(__file__), "..", "amazon", "dataset")
    user_dirs = [d for d in os.listdir(dataset_dir) if d.startswith("user_")]

    # Randomly select 100 users (or use all if less than 100)
    sample_size = min(100, len(user_dirs))
    selected_users = random.sample(user_dirs, sample_size)

    print(f"Testing with {sample_size} randomly selected users")

    # Create CSV file to store results
    csv_file_path = os.path.join(os.path.dirname(__file__), "..", "prompt_lengths.csv")
    with open(csv_file_path, 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        # Write header
        csv_writer.writerow(['User ID', 'Full Prompt Length', 'Important Tokens Length'])

    results = []
    for test_user_id in selected_users:
        print(f"\n===== Generating recommendation prompt for user {test_user_id} =====")
        try:
            full_len, imp_len = generate_recommendation_with_cacheblend(test_user_id)
            print(f"Full prompt length: {full_len}, Important tokens length: {imp_len}")
            results.append((test_user_id, full_len, imp_len))

            # Append result to CSV file
            with open(csv_file_path, 'a', newline='') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow([test_user_id, full_len, imp_len])

        except Exception as e:
            print(f"Error generating recommendation for user {test_user_id}: {e}")
        print(f"===== Finished user {test_user_id} =====")

    # Print summary
    print("\n===== Test Summary =====")
    print(f"Successfully processed {len(results)} out of {sample_size} users")
    if results:
        avg_full_len = sum(r[1] for r in results) / len(results)
        avg_imp_len = sum(r[2] for r in results) / len(results)
        print(f"Average full prompt length: {avg_full_len:.2f}")
        print(f"Average important tokens length: {avg_imp_len:.2f}")
