...
def main():
    ...
    parser = argparse.ArgumentParser(description='TTFT Analysis with configurable data sources')
    parser.add_argument('--data', choices=['long', 'retry', 'both'], default='both',
                       help='Which TTFT data to use: long, retry, or both (default: both)')
    ...
    print("Reading ttft_rawdata_long.csv...")
    df_raw = pd.read_csv('ttft_rawdata_long.csv')

    # Read TTFT results files based on user choice
    df_ttft_combined = pd.DataFrame()
    
    if args.data in ['long', 'both']:
        print("Reading ttft_results_long.csv...")
        df_long = pd.read_csv('ttft_results_long.csv')
        df_ttft_combined = pd.concat([df_ttft_combined, df_long], ignore_index=True)
    
    if args.data in ['retry', 'both']:
        print("Reading ttft_results_retry.csv...")
        df_retry = pd.read_csv('ttft_results_retry.csv')
        df_ttft_combined = pd.concat([df_ttft_combined, df_retry], ignore_index=True)

    # Merge with raw data to get item counts
    print("Merging with raw data to get item counts...")
    df_merged = pd.merge(df_ttft_combined, df_raw[['user_id', 'total_items']], 
                        on='user_id', how='left')

    print(f"Raw data: {len(df_raw)} records")
    print(f"Using data source: {args.data}")
    print(f"Combined TTFT dataset: {len(df_ttft_combined)} records")
    ...
    ax.scatter(x, y, alpha=0.4, s=20, color='lightgreen', label='Data points')
    ...
    ax.set_title(f'TTFT Analysis: Relationship between TTFT and Item Count ({args.data} data)', fontsize=14)
    ...
    # Save the plot
    filename = f'ttft_items_analysis_{args.data}_{timestamp}.png'
