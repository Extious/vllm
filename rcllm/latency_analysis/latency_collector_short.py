...
llm = LLM(model="meta-llama/Llama-3.1-8B-Instruct", gpu_memory_utilization=0.7)
...
def measure_ttft_full_prefill(user_id):
    ...
    try:
        ...
        # Calculate TTLT with full prefill
        ttlf_full_prefill = output[0].metrics.finished_time - output[0].metrics.first_scheduled_time

        print(f"TTFT with full prefill: {ttft_full_prefill}")
        print(f"TTLF with full prefill: {ttlf_full_prefill}")

        return ttft_full_prefill, ttlf_full_prefill, prompt_length

    except Exception as e:
        print(f"Error processing user {user_id}: {e}")
        return None, None, None

def collect_metrics_data(user_ids, output_file="latency_results_short.csv"):
    """Collect TTFT data for multiple users and save to CSV (one test per user)"""
    results = []

    # Create CSV file with headers
    with open(output_file, 'w', newline='') as csvfile:
        fieldnames = ['timestamp', 'user_id', 'ttft_full_prefill', 'ttlf_full_prefill', 'prompt_length']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        for user_id in user_ids:
            print(f"\n{'='*50}")
            ttft_value, ttlf_value, prompt_length = measure_ttft_full_prefill(user_id)
...
def analyze_latency_results(results_file="latency_results_short.csv"):
...
if __name__ == "__main__":
    ...
    user_ids = all_reviewer_ids[:500]  # Use first x reviewers

    # Option 3: Use all reviewers (uncomment to use - WARNING: this will take a long time)
    # user_ids = all_reviewer_ids

    print("=== Latency Collection Script ===")
    print(f"Collecting latency data for {len(user_ids)} user(s) (one test per user)")
    print(f"Selected users: {user_ids[:3]}{'...' if len(user_ids) > 3 else ''}")

    # Collect latency data (one test per user)
    results = collect_metrics_data(user_ids, output_file="latency_results_short.csv")

    # Analyze results
    analyze_latency_results("latency_results_short.csv")  
